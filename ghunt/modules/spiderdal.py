...
async def identify_public_pkgs(
...
async def analyze_single(
    ...
    async with limiter:
        if current_target.site:
            ...
            _, res = await dal.list_statements(
                as_client,
                android_package_name=current_target.package_name,
                android_cert_fingerprint=current_target.certificate,
            )
    ...
    for item in res.statements:
        if item.target.web.site:
            ...
            if clean_site not in sites:
                sites[clean_site] = {
                    "asset": Asset(
                        site=clean_site, package_name=None, certificate=None
                    ),
                    "first_origin": current_target,
                    "origins": set(),
                }
            sites[clean_site]["origins"].add(
                current_target.site if current_target.site else short_pkg_name
            )
        ...
        if item.target.android_app.package_name:
            ...
            if temp_name not in pkgs:
                pkgs[temp_name] = {
                    "asset": Asset(
                        site=None,
                        package_name=item.target.android_app.package_name,
                        certificate=item.target.android_app.certificate.sha_fingerprint,
                    ),
                    "first_origin": current_target,
                    "origins": set(),
                }
            pkgs[temp_name]["origins"].add(
                current_target.site if current_target.site else short_pkg_name
            )
...
async def main(url: str, package: str, fingerprint: str, strict: bool, json_file: Path):
    ...
    if url:
        ...
        for target in temp_targets:
            current_targets.append(
                Asset(site=target, package_name=None, certificate=None)
            )

    if package and fingerprint:
        current_targets.append(
            Asset(site=None, package_name=package, certificate=fingerprint)
        )
    ...
    while current_targets:
        ...
        tmprinter.out(
            f"🕷️ [R{round}]: Investigating {len(current_targets)} targets...",
            style="bold magenta",
        )
        ...
        await asyncio.gather(
            *[
                analyze_single(
                    as_client, digitalassetslink, target, sites, pkgs, visited, limiter
                )
        ...
        # Next candidates
        next_sites = [
            site["asset"] for name, site in sites.items() if not name in visited
        ]
    ...
    tmprinter.clear()
    gb.rc.print(
        f"🕷️ [R{round}]: Investigation done ! {total_scanned} assets scanned.",
        style="bold magenta",
    )

    # Sort
    pkgs_names = {
        x: None for x in set([x["asset"].package_name for x in pkgs.values()])
    }
    await asyncio.gather(
        *[
    ...
    if sites:
        gb.rc.print(
            f"\n🌐 {len(sites)} site{'s' if len(sites) > 1 else ''} found !",
            style="white",
        )
        for site_url, site in sites.items():
            if site["first_origin"]:
                if site["first_origin"].site:
                    gb.rc.print(
                        f"- [deep_sky_blue1][link={site_url}]{site_url}[/link][/deep_sky_blue1] [steel_blue italic](leaked by : {site['first_origin'].site})[/steel_blue italic]"
                    )
                else:
                    gb.rc.print(
                        f"- [deep_sky_blue1][link={site_url}]{site_url}[/link][/deep_sky_blue1] [steel_blue italic](leaked by : {site['first_origin'].package_name})[/steel_blue italic]"
                    )
            else:
                gb.rc.print(
                    f"- [deep_sky_blue1][link={site_url}]{site_url}[/link][/deep_sky_blue1]"
                )
    else:
    ...
    if pkgs:
        gb.rc.print(
            f"\n📦 {len(pkgs_names)} Android package{'s' if len(pkgs) > 1 else ''} found !",
            style="white",
        )
                ...
                if pkg["asset"].package_name == pkg_name:
                    if pkg["asset"].certificate not in fingerprints_cache:
                        if pkg["first_origin"].site:
                            gb.rc.print(
                                f"\t\t- {pkg['asset'].certificate} (leaked by : {pkg['first_origin'].site})",
                                style="steel_blue italic",
                                emoji=False,
                            )
                        else:
                            gb.rc.print(
                                f"\t\t- {pkg['asset'].certificate} (leaked by : {pkg['first_origin'].package_name})",
                                style="steel_blue italic",
                                emoji=False,
                            )
    ...
    if json_file:
        ...
        with open(json_file, "w", encoding="utf-8") as f:
            f.write(
                json.dumps(
                    {"sites": sites, "packages": pkgs}, cls=GHuntEncoder, indent=4
                )
            )
