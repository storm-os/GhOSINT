...
class DriveFile(Parser):
    ...
    def _scrape(self, file_data: Dict[str, any]):
        ...
        self.ancestor_has_augmented_permissions = file_data.get(
...
class DriveCapabilities(Parser):
    ...
    def _scrape(self, capabilities_data: Dict[str, bool]):
        ...
        self.can_change_security_update_enabled = capabilities_data.get(
        ...
        self.can_edit_category_metadata = capabilities_data.get(
            "canEditCategoryMetadata"
        )
        self.can_request_approval = capabilities_data.get("canRequestApproval")
        self.can_move_children_within_drive = capabilities_data.get(
            "canMoveChildrenWithinDrive"
        )
        self.can_move_item_into_team_drive = capabilities_data.get(
            "canMoveItemIntoTeamDrive"
        )
        self.can_move_item_within_drive = capabilities_data.get(
            "canMoveItemWithinDrive"
        )
        self.can_read = capabilities_data.get("canRead")
        self.can_read_category_metadata = capabilities_data.get(
            "canReadCategoryMetadata"
        )
        self.can_remove_children = capabilities_data.get("canRemoveChildren")
        self.can_remove_my_drive_parent = capabilities_data.get(
        ...
        self.can_change_to_file_organizer = capabilities_data.get(
            "canChangeToFileOrganizer"
        )
        ...
        self.can_change_to_reader_on_published_view = capabilities_data.get(
        ...
        self.can_add_encrypted_children = capabilities_data.get(
            "canAddEncryptedChildren"
        )
        self.can_change_copy_requires_writer_permission = capabilities_data.get(
            "canChangeCopyRequiresWriterPermission"
        )
        self.can_change_permission_expiration = capabilities_data.get(
            "canChangePermissionExpiration"
        )
        self.can_change_restricted_download = capabilities_data.get(
            "canChangeRestrictedDownload"
        )
        self.can_change_writers_can_share = capabilities_data.get(
        ...
        self.can_modify_content_restriction = capabilities_data.get(
        ...
        self.can_set_missing_required_fields = capabilities_data.get(
            "canSetMissingRequiredFields"
        )
        self.can_share_as_commenter = capabilities_data.get("canShareAsCommenter")
        self.can_share_as_file_organizer = capabilities_data.get(
        ...
        self.can_share_published_view_as_reader = capabilities_data.get(
            "canSharePublishedViewAsReader"
        )
        self.can_share_to_all_users = capabilities_data.get("canShareToAllUsers")
        self.can_add_folder_from_another_drive = capabilities_data.get(
            "canAddFolderFromAnotherDrive"
        )
        self.can_delete_children = capabilities_data.get("canDeleteChildren")
        self.can_move_item_out_of_team_drive = capabilities_data.get(
            "canMoveItemOutOfTeamDrive"
        )
        self.can_move_item_within_team_drive = capabilities_data.get(
...
class DriveLinkShareMetadata(Parser):
    ...
    def _scrape(self, link_share_metadata_data: Dict[str, any]):
        self.security_update_eligible = link_share_metadata_data.get(
            "securityUpdateEligible"
        )
        self.security_update_enabled = link_share_metadata_data.get(
            "securityUpdateEnabled"
        )
        self.security_update_change_disabled_reason = link_share_metadata_data.get(
            "securityUpdateChangeDisabledReason"
        )
        self.security_update_explicitly_set = link_share_metadata_data.get(
            "securityUpdateExplicitlySet"
        )
...
class DriveApp(Parser):
    ...
    def _scrape(self, app_data: Dict[str, any]):
        ...
        self.requires_authorization_before_open_with = app_data.get(
